...
const TrainerProfileDropdown = ({ avatarUrl, name = 'Dr. Smith', role = 'Lead Trainer' }) => {
    ...
    <>
      <div className="relative" ref={dropdownRef}>
        ...
        >
          ...
          {avatarUrl ? (
            <div
              className="size-10 rounded-full bg-slate-200 border-2 border-primary/20 overflow-hidden bg-cover bg-center flex-shrink-0"
              style={{ backgroundImage: `url('${avatarUrl}')` }}
            />
          ) : (
            <div className="size-10 rounded-full bg-slate-200 border-2 border-primary/20 flex items-center justify-center text-slate-400 flex-shrink-0">
        ...
        {dropdownOpen && (
          <div className="absolute right-0 top-12 z-[200] bg-white border border-slate-200 rounded-xl shadow-xl w-52 py-1 overflow-hidden">
            ...
            {menuItems.map(({ icon, label, onClick, danger }) => (
              ...
              >
                <span className={`material-symbols-outlined`} style={{ fontSize: '18px', color: danger ? '#f43f5e' : '#94a3b8' }}>{icon}</span>
                {label}
              </button>
            ))}
          </div>
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm mx-4 shadow-2xl">
            ...
            <h2 className="text-lg font-bold text-slate-900 text-center mb-2">Are you sure you want to logout?</h2>
            <p className="text-sm text-slate-500 text-center mb-6">Any ongoing AI generations will continue in background</p>
