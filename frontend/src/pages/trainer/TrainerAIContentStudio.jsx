/**
 * TrainerAIContentStudio.jsx
 * Route: /trainer/ai-studio
 *
 * Full navigation flow implemented per Navigation Flow Document (Page 16).
 *
 * Navigation map:
 *  - Generate Audio/Video/Walkthrough  → stays Page 16 (progress modal)
 *  - Navigate Away during generation   → background job continues
 *  - Preview (Play/Watch)              → modal on Page 16
 *  - Download                          → file download (stays Page 16)
 *  - Add to Lesson                     → lesson selector modal → /dashboard/trainer (Page 14)
 *  - Regenerate                        → new generation (stays Page 16)
 *  - Reuse history item                → copies settings (stays Page 16)
 *  - Edit history item                 → inline editor modal (stays Page 16)
 *  - Delete history item               → confirmation → delete (stays Page 16)
 *  - View Queue Details                → expands queue (stays Page 16)
 *  - Pause/Cancel job                  → stays Page 16
 *  - View All Generated Content        → /trainer/content-library?filter=ai-generated (Page 18)
 *  - View Performance                  → /trainer/analytics (Page 17)
 *  - Browse for Enhance                → /trainer/content-library (Page 18) then returns
 *  - Enhance Content                   → stays Page 16
 *  - Dashboard breadcrumb              → /dashboard/trainer (Page 13)
 *  - Back to Course breadcrumb         → /dashboard/trainer (Page 14)
 *  - Avatar Presenter card             → switches to Create Video Explainer tab
 *  - Walkthrough Preview               → modal (stays Page 16)
 *  - Walkthrough Save                  → toast (stays Page 16)
 *  - Walkthrough Add to Lesson         → lesson selector → /dashboard/trainer (Page 14)
 *  - Notification click                → scroll to history (stays Page 16)
 */
...
// ─── Module-level background job store (survives component unmount) ──────────
const jobStore = {
  jobs: [],
  listeners: new Set(),
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        {item.status === "more" && (
          <button className="text-slate-400 flex-shrink-0" onClick={() => setShowActions(v => !v)}>
      ...
      {/* NAV FLOW: done item actions — Play stays Page 16, Download stays Page 16,
          Reuse copies settings stays Page 16, Edit inline stays Page 16,
          Delete confirmation stays Page 16 */}
      {showActions && item.status === "done" && (
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-2">
            {/* NAV: Preview → modal on Page 16 */}
            <button
            ...
            {/* NAV: Download → file download, stays Page 16 */}
            <button
              onClick={() => {
          ...
          <div className="flex items-center gap-2">
            {/* NAV: Reuse → copies settings to form, stays Page 16 */}
            <button
            ...
            {/* NAV: Edit → inline editor modal, stays Page 16 */}
            <button
              onClick={() => onEdit(item)}
              className="flex-1 py-1 bg-white border border-slate-200 rounded text-[10px] font-bold text-slate-600 hover:bg-slate-50"
            >
            ...
            {/* NAV: Delete → confirmation → delete, stays Page 16 */}
            {confirmDelete ? (
              <button
      ...
      {showActions && (item.status === "archived" || item.status === "more") && (
        <div className="mt-3 flex items-center gap-2">
          ...
          {/* NAV: Reuse → copies settings, stays Page 16 */}
          <button
            onClick={() => onReuse(item)}
...
// ─── Preview Modal (stays on Page 16) ────────────────────────────────────────
// NAV FLOW:
//   ▶ Play / ▶ Watch / ▶ Preview → plays inline (stays Page 16)
//   Add to Lesson → opens AddToLessonModal (intermediate, then → Page 14)
//   Regenerate → new generation modal (stays Page 16)
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[300] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8" onClick={e => e.stopPropagation()}>
        ...
        {/* Simulated player — stays Page 16 */}
        <div className="bg-slate-100 rounded-xl flex items-center justify-center h-40 mb-6">
          ...
          {item.type === "walkthrough" && (
            <div className="text-center">
              ...
              <p className="text-xs text-slate-500">{item.meta}</p>
              {/* NAV: Preview Walkthrough — stays Page 16 */}
        ...
        <div className="flex gap-3">
          {/* NAV: Add to Lesson → AddToLessonModal → Course Management (Page 14) */}
          <button
          ...
          {/* NAV: Regenerate → new generation, stays Page 16 */}
          <button
            onClick={() => { onClose(); onRegenerate(item); }}
...
// ─── Add to Lesson Modal ──────────────────────────────────────────────────────
// NAV FLOW: Confirm → navigate to Course Management (Page 14) with context
    ...
    <div className="fixed inset-0 bg-black/60 z-[400] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8" onClick={e => e.stopPropagation()}>
        ...
        <p className="text-sm text-slate-500 mb-4">
          Select a lesson to add <strong>{item?.title}</strong>:
        </p>
        ...
        <div className="flex gap-3">
          ...
          {/* NAV: Confirm → Course Management (Page 14) */}
          <button
...
// ─── Walkthrough Preview Modal (stays Page 16) ───────────────────────────────
// NAV FLOW: Preview Walkthrough → modal on Page 16
...
// ─── Generation Progress Modal (stays Page 16) ───────────────────────────────
// NAV FLOW:
//   While generating → shows progress (stays Page 16)
//   "Navigate Away" → starts background job, processing continues
//   "View in History" (done) → closes modal, stays Page 16
    ...
    <div className="fixed inset-0 bg-black/50 z-[200] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8 text-center">
        ...
        <h3 className="text-lg font-bold mb-1">
          {done ? `${tool} Ready!` : `Generating ${tool}...`}
        </h3>
        ...
        {!done && (
          /* NAV: Navigate Away → background job continues, user can go to other pages */
          <button
        ...
        {done && (
          /* NAV: View in History → closes modal, stays Page 16, scrolls to history */
          <button
            onClick={onDone}
...
// NAV FLOW:
//   View Queue Details → expands inline (stays Page 16)
//   Click job item → shows details (stays Page 16)
//   Pause / Resume → stays Page 16
//   Cancel → stays Page 16
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
      <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpanded(v => !v)}>
        ...
        {/* NAV: View Queue Details → expands, stays Page 16 */}
        <button
          className="text-[#137fec] text-xs font-bold flex items-center gap-1"
      ...
      {expanded && (
        <div className="mt-4 space-y-3">
          {jobs.map(job => (
            /* NAV: Click individual queue item → shows details, stays Page 16 */
            <div key={job.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
              ...
              <div className="flex gap-1 flex-shrink-0">
                {job.status !== "done" && (
                  /* NAV: Pause / Resume → stays Page 16 */
                  <button
                    onClick={() => job.paused ? onPause(job.id, false) : onPause(job.id, true)}
...
// ─── Enhance Existing Content Card ───────────────────────────────────────────
// NAV FLOW:
//   Browse... → /trainer/content-library (Page 18), returns with selection
//   Enhance Content → stays Page 16, shows progress / options
//   Preview enhanced → stays Page 16
//   Apply to Lesson → Course Management (Page 14)
//   Save Separately → stays Page 16
function EnhanceContentCard({ onBrowse, onEnhance }) {
  ...
  const handleEnhanceClick = () => {
    ...
    setTimeout(() => {
      setEnhancing(false);
      setEnhanced({
        title: `Enhanced: ${selectedContent.name.substring(0, 20)}`,
        summary: enhancements.summary,
        concepts: enhancements.concepts,
        quiz: enhancements.quiz,
      });
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
      ...
      <div className="flex gap-2 mb-4">
        ...
        {/* NAV: Browse → Content Library (Page 18), returns with selection */}
        <button
          onClick={() => onBrowse(setSelectedContent)}
      ...
      {selectedContent && !enhanced && (
        <div className="space-y-2 mb-4">
          ...
          {[
            ...
            { key: "quiz", label: "Assessment Quiz" },
          ].map(({ key, label }) => (
      ...
      {/* NAV: After enhancement — Preview stays Page 16, Apply to Lesson → Page 14, Save Separately stays Page 16 */}
      {enhanced && (
      ...
      {/* NAV: Enhance Content → stays Page 16 (processing) */}
      <button
      ...
      >
        {enhancing ? (
          <>
            <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full" style={{ animation: "spin 1s linear infinite" }} />
            Enhancing...
          </>
        ) : (
          <>
            <Icon name="auto_fix_high" className="text-lg" />
            {selectedContent ? "Enhance Content" : "Select Content First"}
          </>
        )}
      </button>
    </div>
...
// ─── Notification Toast ───────────────────────────────────────────────────────
// NAV FLOW: Click notification action → returns to AI Studio with context (scroll to history)
function Toast({ message, onClose, onAction, actionLabel }) {
  ...
  return (
    <div className="fixed bottom-24 right-6 z-[500] bg-slate-900 text-white px-5 py-4 rounded-2xl shadow-2xl flex items-center gap-4 max-w-sm">
      ...
      {actionLabel && (
        /* NAV: Click notification → scroll to history (stays Page 16) */
        <button onClick={onAction} className="text-[#137fec] text-xs font-bold whitespace-nowrap">
          {actionLabel}
        </button>
      )}
...
// ─── Inline Edit Modal (stays Page 16) ───────────────────────────────────────
    ...
    <div className="fixed inset-0 bg-black/60 z-[400] flex items-center justify-center p-4"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8" onClick={e => e.stopPropagation()}>
        ...
        <input
          ...
          className="w-full bg-slate-50 border border-slate-200 rounded-xl text-sm p-3 mb-4"
        ...
        <textarea
          ...
          className="w-full h-24 bg-slate-50 border border-slate-200 rounded-xl text-sm p-3 resize-none mb-6"
          placeholder="Add notes..."
          style={{ fontFamily: "'Lexend', sans-serif" }}
        />
        <div className="flex gap-3">
          <button onClick={onClose}
            className="flex-1 py-2.5 bg-slate-100 text-slate-700 rounded-xl text-sm font-bold">
            Cancel
          </button>
          {/* NAV: Save → stays Page 16 */}
          <button
            onClick={() => onSave({ ...item, title })}
            className="flex-1 py-2.5 bg-[#137fec] text-white rounded-xl text-sm font-bold hover:bg-[#0f6fd4]"
          >
            Save
          </button>
...
export default function TrainerAIContentStudio() {
  ...
  // NAV FLOW: Return from Content Library with selected content
  useEffect(() => {
    if (location?.state?.returnFrom === "content-library" && location?.state?.selectedContent) {
      // Content pre-selected via browse flow — would wire into EnhanceContentCard
    }
  }, [location]);
  ...
  const addStep = () =>
    setSteps(prev => [...prev, { id: Date.now(), label: `Step ${prev.length + 1}: New Step` }]);

  // ── Generate handler
  // NAV FLOW: Click Generate → generation modal (stays Page 16)
  ...
  // NAV FLOW: Generation done → add to history, close modal, stays Page 16, scroll to history
  const onGenerateDone = () => {
    ...
    const newItem = {
      ...
      meta: `${formatDuration(duration)} • ~1 MB`,
      status: "done",
    };
  ...
  // NAV FLOW: Navigate Away during generation → background job continues in module store
  const onGenerateNavigateAway = () => {
    ...
    startBackgroundJob(type, title, (jobId) => {
      ...
      setHistory(prev => ({ ...prev, today: [newItem, ...prev.today] }));
      // NAV FLOW: Notification "Video ready!" → click returns focus to AI Studio history
      addToast(
        `${type.charAt(0).toUpperCase() + type.slice(1)} ready! "${title}"`,
        "View",
        () => {
  ...
  // NAV: Save from editor → stays Page 16
  ...
  // NAV: Reuse → copies settings, switches tab, stays Page 16
  const handleReuse = (item) => {
    const tabMap = {
      audio: "Generate Audio Summary",
      video: "Create Video Explainer",
      walkthrough: "Build Interactive Walkthrough",
    };
  ...
  // NAV: Delete → confirmation (in HistoryItem) → delete, stays Page 16
  ...
  // NAV: Add to Lesson (from Preview modal) → AddToLessonModal
  const handleAddToLesson = (item) => {
    setPreviewItem(null);
    setAddToLessonItem(item);
  };

  // NAV: Confirm lesson selection → Course Management (Page 14)
  ...
  // NAV: Regenerate → new generation modal, stays Page 16
  ...
  // NAV: View Performance → Student Analytics (Page 17)
  const handleViewPerformance = () => {
    navigate("/trainer/analytics", { state: { filter: "ai-content", from: "ai-studio" } });
  };

  // NAV: View All Generated Content → Content Library (Page 18) filtered to AI-generated
  const handleViewAllHistory = () => {
    navigate("/trainer/content-library", { state: { filter: "ai-generated", from: "ai-studio" } });
  };

  // NAV: Browse for Enhance → Content Library (Page 18), returns with selection
  // In real app would navigate and return; here we simulate content selection
  const handleBrowseContent = (setSelectedFn) => {
    // Real: navigate("/trainer/content-library", { state: { selectMode: true, returnTo: "/trainer/ai-studio" } });
    // Simulated: immediately set a mock content selection
    const mockContent = { name: "Python Functions - Lesson 2.pdf", id: "c1" };
    setSelectedFn(mockContent);
  };

  // NAV: Enhance Content → stays Page 16
  const handleEnhanceContent = (content, enhancements) => {
    const newItem = {
      id: Date.now(),
      type: "audio",
      title: `Enhanced: ${content.name.substring(0, 20)}`,
      meta: "AI Enhanced • ~1 MB",
      status: "done",
    };
    setHistory(prev => ({ ...prev, today: [newItem, ...prev.today] }));
    addToast(
      "Content enhanced! Preview or apply it to a lesson.",
      "View",
      () => historyRef.current?.scrollIntoView({ behavior: "smooth" })
    );
  };

  // ── Queue handlers (all stay Page 16)
  const handlePauseJob = (id, shouldPause) => {
    if (shouldPause) jobStore.pause(id); else jobStore.resume(id);
  };
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after { box-sizing: border-box; }
        ...
        textarea:focus, input[type="text"]:focus { outline: none; border-color: #137fec; box-shadow: 0 0 0 2px rgba(19,127,236,0.2); }
        input[type=range] { -webkit-appearance: none; appearance: none; height: 6px; border-radius: 3px; background: #e2e8f0; }
        input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; width: 16px; height: 16px; border-radius: 50%; background: #137fec; }
      `}</style>

      {/* ── Modals ── */}

      {/* NAV: Generation modal — stays Page 16; Navigate Away → background job */}
      {generatingTool && (
        <GeneratingModal
          tool={generatingTool}
          onDone={onGenerateDone}
          onNavigateAway={onGenerateNavigateAway}
        />
      )}

      {/* NAV: Preview modal — stays Page 16 */}
      {previewItem && (
        <PreviewModal
          item={previewItem}
      ...
      {/* NAV: Add to Lesson modal — confirm → Course Management (Page 14) */}
      {addToLessonItem && (
        <AddToLessonModal
          item={addToLessonItem}
          onClose={() => setAddToLessonItem(null)}
          onConfirm={handleLessonConfirm}
        />
      )}

      {/* NAV: Inline edit modal — stays Page 16 */}
      {editingItem && (
        <EditItemModal
          item={editingItem}
      ...
      {/* NAV: Walkthrough preview modal — stays Page 16 */}
      {showWalkthroughPreview && (
        <WalkthroughPreviewModal
          steps={steps}
          onClose={() => setShowWalkthroughPreview(false)}
        />
      )}
              ...
              <nav className="hidden md:flex items-center gap-6">
                {["Creation Hub", "Library", "Templates", "Analytics"].map(link => (
                  <button
                    ...
                    onClick={() => {
                      setActiveNav(link);
                      // NAV: Analytics tab → Student Analytics (Page 17)
            ...
            <div className="flex items-center gap-4">
              <div className="relative hidden lg:block">
                ...
                <input
                  ...
                  className="bg-slate-100 border border-transparent rounded-lg py-2 pl-10 pr-4 text-sm w-64 transition-all"
          ...
          <main className="flex-1 overflow-y-auto custom-scroll p-8" style={{ paddingBottom: walkthroughOpen ? "320px" : "32px" }}>
            <div className="max-w-6xl mx-auto">
              ...
              <div className="flex items-center gap-2 text-sm text-slate-500 mb-4">
                {/* NAV: Dashboard breadcrumb → Trainer Dashboard (Page 13) */}
                <button
                ...
                {/* NAV: Back to Course breadcrumb → Course Management (Page 14) */}
                <button
                  onClick={() => navigate("/dashboard/trainer")}
                  className="hover:text-[#137fec] transition-colors font-medium"
                >
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div className="xl:col-span-2 space-y-6">
                    ...
                    <div className="p-6 space-y-6">
                      <div>
                        ...
                        <textarea
                          ...
                          className="w-full h-40 bg-slate-50 border border-slate-200 rounded-xl text-sm p-4 placeholder:text-slate-400 resize-none"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-4 pt-2">
                              <input
                                type="range" min={0} max={100} value={duration}
                      ...
                      <div className="flex items-center justify-between pt-4">
                        <div className="flex items-center gap-4">
                          <div className="flex -space-x-2">
                            {[{ bg: "bg-indigo-300", l: "S" }, { bg: "bg-rose-300", l: "J" }, { bg: "bg-amber-300", l: "M" }].map((a, i) => (
                              <div key={i} className={`w-8 h-8 rounded-full border-2 border-white ${a.bg} flex items-center justify-center text-white text-[10px] font-bold`}>
                                {a.l}
                              </div>
                            ))}
                        ...
                        {/* NAV: Generate → generation modal (stays Page 16) */}
                        <button
                          onClick={handleGenerate}
                          className="bg-[#137fec] text-white font-bold py-3 px-8 rounded-xl shadow-lg shadow-[#137fec]/30 hover:bg-[#0f6fd4] hover:shadow-[#137fec]/40 transition-all flex items-center gap-2"
                        >
                  ...
                  {/* Secondary cards */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* NAV: Avatar Presenter → switches to Create Video Explainer tab */}
                    <div
                      className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:border-[#137fec]/40 transition-all cursor-pointer group"
      ...
      {/* WALKTHROUGH DRAWER */}
      {/* NAV FLOW:
           - Open/close drawer stays Page 16
           - Edit steps inline stays Page 16
           - Preview Walkthrough → WalkthroughPreviewModal (stays Page 16)
           - Save → toast (stays Page 16)
           - Add to Lesson → AddToLessonModal → Course Management (Page 14)
      */}
      <div
        className="fixed bottom-0 bg-white border-t border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] transition-transform duration-500 z-[60]"
        style={{
          transform: walkthroughOpen ? "translateY(0)" : "translateY(85%)",
          left: "256px", // account for TrainerSidebar width
          right: 0,
        }}
      >
        <div className="max-w-6xl mx-auto px-8 py-4">
          ...
          {/* Inline step editor — stays Page 16 */}
          ...
          <div className="flex gap-3 pb-4">
            {/* NAV: Preview Walkthrough → WalkthroughPreviewModal (stays Page 16) */}
            <button
              onClick={() => setShowWalkthroughPreview(true)}
              className="px-5 py-2.5 bg-slate-100 text-slate-700 rounded-xl text-sm font-bold hover:bg-slate-200 transition-all"
            >
              Preview Walkthrough
            </button>
            {/* NAV: Save → toast, stays Page 16 */}
            <button
              onClick={() => addToast("Walkthrough saved!", null, null)}
            ...
            {/* NAV: Add to Lesson → AddToLessonModal → Course Management (Page 14) */}
            <button
              onClick={() => setAddToLessonItem({ title: "Interactive Walkthrough", type: "walkthrough" })}
